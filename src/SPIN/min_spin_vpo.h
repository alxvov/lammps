...
MinimizeStyle(spin/vpo, MinSpinVPO)

#else

#ifndef LMP_MIN_SPIN_VPO_H
#define LMP_MIN_SPIN_VPO_H

#include "min.h"

namespace LAMMPS_NS {

class MinSpinVPO: public Min {
 public:
  MinSpinVPO(class LAMMPS *);
  virtual ~MinSpinVPO();
  void init();
  void setup_style();
  void reset_vectors();
 ...
 private:
  ...
  double *eff_vel;  		// effective velocity
  double eff_mass;      // effective mass
  double eff_dt0;       // effective time step
