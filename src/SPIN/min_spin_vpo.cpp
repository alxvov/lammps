...
static const char cite_minstyle_spin_vpo[] =
  "min_style spin/vpo command:\n\n"
  ...
  Min(lmp), g_old(NULL), g_cur(NULL), p_s(NULL), eff_vel(NULL), sp_copy(NULL)
{
  ...
  use_line_search = 0;  // no line search as default option
...
MinSpinVPO::~MinSpinVPO()
{
  memory->destroy(g_old);
  memory->destroy(g_cur);
  memory->destroy(p_s);
  memory->destroy(eff_vel);
...
void MinSpinVPO::init()
{
  ...
  memory->grow(g_old,3*nlocal_max,"min/spin/vpo:g_old");
  memory->grow(g_cur,3*nlocal_max,"min/spin/vpo:g_cur");
  memory->grow(p_s,3*nlocal_max,"min/spin/vpo:p_s");
  memory->grow(eff_vel,3*nlocal_max,"min/spin/vpo:eff_vel");

  if (use_line_search)
    memory->grow(sp_copy,nlocal_max,3,"min/spin/vpo:sp_copy");
}

/* ---------------------------------------------------------------------- */

void MinSpinVPO::setup_style()
{
  double **v = atom->v;
  ...
  if (!atom->sp_flag)
    error->all(FLERR,"min spin/vpo requires atom/spin style");
...
int MinSpinVPO::modify_param(int narg, char **arg)
{
...
int MinSpinVPO::iterate(int maxiter)
{
  int nlocal = atom->nlocal;
  bigint ntimestep;
  double fmdotfm,fmsq;
...
/* ----------------------------------------------------------------------
   ...
   H. J'{o}nsson et. al, Classical and Quantum Dynamics
   in Condesed Phase Simulations (1998)
---------------------------------------------------------------------- */

void MinSpinVPO::calc_search_direction()
{
  int nlocal = atom->nlocal;
  double gdotvel = 0.0;
  double g2 = 0.0;
  ...
  if (local_iter == 0){ 	// steepest descent direction
    for (int i = 0; i < 3 * nlocal; i++) {
      eff_vel[i] = 0.0;
      p_s[i] = -g_cur[i] * factor * eff_dt0*eff_dt0/(2.0*eff_mass);
      g_old[i] = g_cur[i] * factor;
    }
  } else {

    for (int i = 0; i < 3 * nlocal; i++) {
      eff_vel[i] = eff_vel[i] - 0.5 * ( g_cur[i] +  g_old[i] ) * eff_dt0/eff_mass;
      g2 += g_cur[i] * g_cur[i];
      gdotvel += eff_vel[i] *  g_cur[i];
    }
    ...
    MPI_Allreduce(&gdotvel,&gdotvel_global,1,MPI_DOUBLE,MPI_SUM,world);

    // Sum over all replicas. Good for GNEB.
    
    if (nreplica > 1) {
      g2 = g2_global * factor;
      gdotvel = gdotvel_global * factor;
      MPI_Allreduce(&g2,&g2_global,1,MPI_DOUBLE,MPI_SUM,universe->uworld);
      MPI_Allreduce(&gdotvel,&gdotvel_global,1,MPI_DOUBLE,MPI_SUM,universe->uworld);
    }
    if (fabs(gdotvel_global) < 1.0e-60) beta = 0.0;
    else beta = gdotvel_global / g2_global;

    if (beta > 0.0) beta =0.0;
    for (int i = 0; i < 3 * nlocal; i++) {
      eff_vel[i] = beta * g_cur[i];
    }
    // calculate conjugate direction
    
    for (int i = 0; i < 3 * nlocal; i++) {
      p_s[i] = eff_dt0 * (eff_vel[i] - g_cur[i] * eff_dt0 / (2.0 * eff_mass) ) * factor;
      g_old[i] = g_cur[i] * factor;
    }
  }
...
void MinSpinVPO::rodrigues_rotation(const double *upp_tr, double *out)
{
...
void MinSpinVPO::vm3(const double *m, const double *v, double *out)
{
  for(int i = 0; i < 3; i++){
    out[i] = 0.0;
...
void MinSpinVPO::make_step(double c, double *energy_and_der)
{
  double p_scaled[3];
  int nlocal = atom->nlocal;
  double rot_mat[9]; // exponential of matrix made of search direction
...
int MinSpinVPO::calc_and_make_step(double a, double b, int index)
{
...
int MinSpinVPO::adescent(double phi_0, double phi_j){
